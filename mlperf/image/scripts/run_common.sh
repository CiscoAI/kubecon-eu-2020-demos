...
if [ $# -lt 1 ]; then
    echo "usage: $0 tf|tfserving|onnxruntime|pytorch|tflite [resnet50|mobilenet|ssd-mobilenet|ssd-resnet34] [cpu|gpu]"
    exit 1
fi
...
for i in $* ; do
    case $i in
       tf|tfserving|onnxruntime|tflite|pytorch) backend=$i; shift;;
...
# tfserving
#
if [ $name == "ssd-mobilenet-tfserving" ] ; then
    model_path="$MODEL_DIR/ssd_mobilenet_v1_coco_2018_01_28.pb"
    profile=ssd-mobilenet-tfserving
